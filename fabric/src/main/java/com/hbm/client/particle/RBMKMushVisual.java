...
public final class RBMKMushVisual implements EffectVisual<ParticleRBMKMush>, SimpleTickableVisual {
    private final ParticleRBMKMush particle;
    private final ParticleInstance instance;
    private final Vec3i origin;

    public RBMKMushVisual(VisualizationContext context, ParticleRBMKMush particle) {
        this.particle = particle;
        this.origin = context.renderOrigin();
        this.instance =
                context.instancerProvider()
                        .instancer(ParticleInstance.TYPE, ParticleModels.ADDITIVE_NO_FOG)
                        .createInstance();
        writeTick();
    }
