...
public class EntityChlorineFX extends Entity {

    private int particleAge;
    private int maxAge;

    public EntityChlorineFX(EntityType<? extends EntityChlorineFX> type, Level level) {
        super(type, level);
        noPhysics = false;
    }

    public EntityChlorineFX(
            Level level,
            double x,
            double y,
            double z,
            ...
            double motionY,
            double motionZ) {
        this(ModEntities.CHLORINE_FX.get(), level);
        setPos(x, y, z);
    ...
    public void tick() {
        ...
        if (maxAge < 700) maxAge = random.nextInt(101) + 700;

        if (random.nextInt(50) == 0) ExplosionChaos.poison(level(), getX(), getY(), getZ(), 2D);
