...
import com.hbm.inventory.recipes.ReformingRecipe;
...
public class BlockEntityMachineCatalyticReformer extends BlockEntityMachineBase
    ...
    public static final int OUTPUT_TANK_COUNT = 3;
    public static final int INPUT_TANK_CAPACITY = 64_000;
    public static final int OUTPUT_TANK_CAPACITY = 24_000;

    private static final String[] PERSISTENT_KEYS = {"input", "o1", "o2", "o3"};
    ...
    public BlockEntityMachineCatalyticReformer(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CATALYTIC_REFORMER.get(), pos, state, SLOT_COUNT);
        tanks[0] = new FluidTankNTM(NTMFluids.NAPHTHA, INPUT_TANK_CAPACITY);
        tanks[1] = new FluidTankNTM(NTMFluids.REFORMATE, OUTPUT_TANK_CAPACITY);
        tanks[2] = new FluidTankNTM(NTMFluids.PETROLEUM, OUTPUT_TANK_CAPACITY);
        tanks[3] = new FluidTankNTM(NTMFluids.HYDROGEN, OUTPUT_TANK_CAPACITY);
        receiving = new FluidTankNTM[] {tanks[0]};
        sending = new FluidTankNTM[] {tanks[1], tanks[2], tanks[3]};
    }
    ...
    private void reform() {
        ReformingRecipe recipe =
                ReformingRecipes.INSTANCE.getRecipeForInput(tanks[0].getTankType());
        if (recipe == null) {
        ...
        FluidStackNTM out3 = recipe.outputFluid[2];

        tanks[1].setTankType(out1.type());
        tanks[2].setTankType(out2.type());
        tanks[3].setTankType(out3.type());
        ...
        if (tanks[1].getFill() + out1.amount() > tanks[1].getMaxFill()) return;
        if (tanks[2].getFill() + out2.amount() > tanks[2].getMaxFill()) return;
        if (tanks[3].getFill() + out3.amount() > tanks[3].getMaxFill()) return;

        tanks[0].setFill((int) (tanks[0].getFill() - in.amount()));
        tanks[1].setFill((int) (tanks[1].getFill() + out1.amount()));
        tanks[2].setFill((int) (tanks[2].getFill() + out2.amount()));
        tanks[3].setFill((int) (tanks[3].getFill() + out3.amount()));
    ...
    public AbstractContainerMenu createMenu(
        ...
        return new MenuMachineCatalyticReformer(containerId, playerInventory, this);
    }
    ...
    protected void loadAdditional(ValueInput input) {
        ...
        input.child("o1").ifPresent(tanks[1]::deserialize);
        input.child("o2").ifPresent(tanks[2]::deserialize);
        input.child("o3").ifPresent(tanks[3]::deserialize);
    }
    ...
    protected void saveAdditional(ValueOutput output) {
        ...
        tanks[1].serialize(output.child("o1"));
        tanks[2].serialize(output.child("o2"));
        tanks[3].serialize(output.child("o3"));
    }
