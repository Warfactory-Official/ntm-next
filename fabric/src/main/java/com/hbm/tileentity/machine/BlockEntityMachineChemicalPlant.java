...
public class BlockEntityMachineChemicalPlant extends BlockEntityMachineBase
    ...
    public static final int SLOT_FLUID_IN_RESULT = 13;
    public static final int SLOT_FLUID_OUT_START = 16;
    public static final int SLOT_FLUID_OUT_RESULT = 19;
    ...
    private static final int[] ITEM_OUT_SLOTS = {
        SLOT_ITEM_OUT_START, SLOT_ITEM_OUT_START + 1, SLOT_ITEM_OUT_START + 2
    };
    private static final int[] ACCESSIBLE_SLOTS = {4, 5, 6, 7, 8, 9};
    ...
    public static Consumer<BlockEntityMachineChemicalPlant> CLIENT_SOUND = be -> {};

    @SyncField(units = 1L << 4)
    public final FluidTankNTM[] inputTanks = new FluidTankNTM[TANK_COUNT];

    @SyncField(units = 1L << 5)
    public final FluidTankNTM[] outputTanks = new FluidTankNTM[TANK_COUNT];
    ...
    public BlockEntityMachineChemicalPlant(BlockPos pos, BlockState state) {
        super(ModBlockEntities.CHEMICALPLANT.get(), pos, state, SLOT_COUNT);
    ...
    public boolean canPlaceItem(int slot, ItemStack stack) {
        ...
        if (slot >= SLOT_FLUID_IN_START && slot < SLOT_FLUID_IN_RESULT) return true;
        if (slot >= SLOT_FLUID_OUT_START && slot < SLOT_FLUID_OUT_RESULT) return true;
    ...
    public boolean canTakeItemThroughFace(int slot, ItemStack stack, Direction side) {
        return (slot >= SLOT_ITEM_OUT_START && slot < SLOT_FLUID_IN_START)
                || module.isSlotClogged(slot);
    }
    ...
    public void tickServer() {
        ...
        for (int i = 0; i < TANK_COUNT; i++) {
            changed |=
                    inputTanks[i].loadTank(
                            SLOT_FLUID_IN_START + i, SLOT_FLUID_IN_RESULT + i, inventory);
            changed |=
                    outputTanks[i].unloadTank(
                            SLOT_FLUID_OUT_START + i, SLOT_FLUID_OUT_RESULT + i, inventory);
        }

        module.update(speed, pow, true, inventory.get(SLOT_SCHEMATIC));
        isProgressing = module.didProcess;

        if (module.didProcess) {
            ItemSwordMeteorite.upgrade(
                    inventory,
                    SLOT_BATTERY,
                    ModItems.METEORITE_SWORD_MACHINED,
                    ModItems.METEORITE_SWORD_TREATED);
        }

        if (changed || module.markDirty || power != prevPower) setChanged();
    ...
    public @Nullable String provideRORValue(String name) {
        ...
        if ((PREFIX_VALUE + "active").equals(name)) return "" + (module.didProcess ? 1 : 0);
        return null;
    }
    ...
    public @Nullable String runRORFunction(String name, String[] params) {
        if ((PREFIX_FUNCTION + "setrecipe").equals(name) && params.length == 1) {
            module.setRecipe(params[0], true);
            setChanged();
        }
        return null;
    }
    ...
    public AbstractContainerMenu createMenu(
        ...
        return new MenuMachineChemicalPlant(containerId, playerInventory, this);
    }
