...
// SPDX-License-Identifier: BSD-3-Clause AND LGPL-3.0-only
...
public class BusAnimationKeyframe {
    ...
    public BusAnimationKeyframe(double value, int duration) {
        ...
        this.originalDuration =
                this.duration = (int) (duration / Math.max(0.001D, GunVisualConfig.animationSpeed));
    }
    ...
    public double interpolate(double startTime, double currentTime, BusAnimationKeyframe previous) {
        ...
        if (previous.interpolationType == IType.BEZIER) {
            ...
            switch (previous.easingType) {
                case EASE_IN:
                    return BLI_easing_back_ease_in(time, begin, change, duration, previous.back);
                case EASE_IN_OUT:
                    return BLI_easing_back_ease_in_out(
                            time, begin, change, duration, previous.back);
                default:
                    return BLI_easing_back_ease_out(time, begin, change, duration, previous.back);
            }
            ...
            switch (previous.easingType) {
                case EASE_IN:
                    return BLI_easing_elastic_ease_in(
                            time, begin, change, duration, previous.amplitude, previous.period);
                case EASE_IN_OUT:
    ...
    private double time(double start, double end, double duration) {
        if (interpolationType == IType.SIN_UP)
            return -Math.sin(((end - start) / duration * Math.PI + Math.PI) / 2) + 1;
        if (interpolationType == IType.SIN_DOWN)
            return Math.sin((end - start) / duration * Math.PI / 2);
        if (interpolationType == IType.SIN_FULL)
            return (-Math.cos((end - start) / duration * Math.PI) + 1) / 2D;
    ...
    double BLI_easing_back_ease_in(
            double time, double begin, double change, double duration, double overshoot) {
        time /= duration;
        return change * time * time * ((overshoot + 1) * time - overshoot) + begin;
    }

    double BLI_easing_back_ease_out(
            double time, double begin, double change, double duration, double overshoot) {
        time = time / duration - 1;
        return change * (time * time * ((overshoot + 1) * time + overshoot) + 1) + begin;
    }

    double BLI_easing_back_ease_in_out(
            double time, double begin, double change, double duration, double overshoot) {
        overshoot *= 1.525f;
    ...
    double BLI_easing_bounce_ease_in_out(
            double time, double begin, double change, double duration) {
        if (time < duration / 2) {
    ...
    double elastic_blend(
            double time, double change, double duration, double amplitude, double s, double f) {
        if (change != 0) {
    ...
    double BLI_easing_elastic_ease_in(
            double time,
            double begin,
            double change,
            double duration,
        ...
        return (-f
                        * (amplitude
                                * Math.pow(2, 10 * time)
                                * Math.sin((time * duration - s) * (2 * Math.PI) / period)))
                + begin;
    }

    double BLI_easing_elastic_ease_out(
            double time,
            double begin,
            double change,
            double duration,
    ...
    double BLI_easing_elastic_ease_in_out(
            double time,
            double begin,
            double change,
            double duration,
