...
@SyncSlots(
        value = {BlockEntityCompactLauncher.SLOT_MISSILE},
        units = 1L << 4)
public class BlockEntityCompactLauncher extends BlockEntityMachineBase
    ...
    public static final int TANK_CAPACITY = 25_000;
    public static final int SOLID_PER_ITEM = 250;

    public static final PartSize PAD_SIZE = PartSize.SIZE_10;
    ...
    public BlockEntityCompactLauncher(BlockPos pos, BlockState state) {
        super(ModBlockEntities.SMALL_LAUNCHER.get(), pos, state, SLOT_COUNT);
    }
    ...
    public void tickClient() {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            double motionX = alongZ ? 0D : level.getRandom().nextGaussian() * 0.5D;
            double motionZ = alongZ ? level.getRandom().nextGaussian() * 0.5D : 0D;
            level.addParticle(
    ...
    private static ItemCustomMissilePart fuselageOf(ItemStack missile) {
    ...
    public boolean isMissileValid(ItemStack missile) {
        ...
        return fuselage != null && fuselage.top == PAD_SIZE;
    }
    ...
    public int solidState(ItemStack missile) {
        ItemCustomMissilePart fuselage = fuselageOf(missile);
        if (fuselage == null || fuselage.fuelType() != ItemCustomMissilePart.FuelType.SOLID)
            return -1;
    ...
    public void readSyncUnit(int unit, ByteBuf input) {
        switch (unit) {
            ...
            case 1 -> solid = input.readInt();
            case 2 -> fuelTank.packetDeserialize(input);
            case 3 -> oxidizerTank.packetDeserialize(input);
            case 4 ->
                    loadedMissile =
                            MissileStruct.STREAM_CODEC.decode(registryBuf(input)).sanitised();
    ...
    protected Component getDefaultName() {
        return Component.translatable("container.compactLauncher");
    }
    ...
    public AbstractContainerMenu createMenu(int containerId, Inventory inventory, Player player) {
        return new MenuCompactLauncher(containerId, inventory, this);
    }
}
