...
public final class ScreenRBMKIndicator extends Screen {

    private static final int X_SIZE = 256;
    private static final int Y_SIZE = 258;
    private static final int LAMPS = BlockEntityRBMKIndicator.INDICATORS;
    ...
    private static final int SWATCH_X = 102;
    private static final int SWATCH_SIZE = 8;
    private static final int SAVE_X = 209;
    private static final int SAVE_Y = 8;
    private static final int MAX_CHAN_LENGTH = 15;

    private final BlockEntityRBMKIndicator indicator;
    private final Identifier texture = Library.id("textures/gui/machine/gui_rbmk_indicator.png");

    private final EditBox[] color = new EditBox[LAMPS];
    private final EditBox[] label = new EditBox[LAMPS];
    private final EditBox[] rtty = new EditBox[LAMPS];
    private final EditBox[] min = new EditBox[LAMPS];
    private final EditBox[] max = new EditBox[LAMPS];
    private final boolean[] active = new boolean[LAMPS];
    private final boolean[] polling = new boolean[LAMPS];

    private int guiLeft;
    private int guiTop;

    public ScreenRBMKIndicator(BlockEntityRBMKIndicator indicator) {
    ...
    protected void init() {
        ...
        for (int i = 0; i < LAMPS; i++) {
            BlockEntityRBMKIndicator.IndicatorUnit unit = indicator.indicators[i];
            String col = Integer.toHexString(unit.color);
            while (col.length() < 6) col = "0" + col;

            color[i] =
                    field(guiLeft + 27 + oX, guiTop + 37 + oY + i * ROW_PITCH, 72 - oX * 2, col, 6);
            label[i] =
                    field(
                            guiLeft + 175 + oX,
                            guiTop + 37 + oY + i * ROW_PITCH,
                            72 - oX * 2,
                            unit.label,
                            15);
            rtty[i] =
    ...
    public void extractRenderState(
        ...
        Component name = Component.translatable("container.rbmkIndicator");
        graphics.text(
                font,
                name,
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        for (int i = 0; i < LAMPS; i++) {
            if (guiLeft + ACTIVE_X <= x
                    ...
                    && guiTop + i * ROW_PITCH + 36 < y
                    && guiTop + i * ROW_PITCH + 36 + ACTIVE_SIZE >= y) {
                active[i] = !active[i];
            ...
            if (guiLeft + POLLING_X <= x
                    ...
                    && guiTop + i * ROW_PITCH + 35 < y
                    && guiTop + i * ROW_PITCH + 35 + TOGGLE_SIZE >= y) {
                polling[i] = !polling[i];
    ...
    private void sendSettings() {
        ...
        Services.NETWORK.sendToServer(new NbtControlPayload(indicator.getBlockPos(), data));
    }
