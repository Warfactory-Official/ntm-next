...
public final class ScreenRBMKNumitron extends Screen {

    private static final int X_SIZE = 256;
    private static final int Y_SIZE = 150;
    private static final int DISPLAYS = BlockEntityRBMKNumitron.DISPLAYS;
    private static final int ROW_PITCH = 54;
    private static final int ACTIVE_X = 124;
    private static final int ACTIVE_SIZE = 16;
    private static final int POLLING_X = 159;
    private static final int SHORTEN_X = 195;
    private static final int LEADING_X = 231;
    ...
    private final BlockEntityRBMKNumitron numitron;
    private final Identifier texture = Library.id("textures/gui/machine/gui_rbmk_numitron.png");
    ...
    public ScreenRBMKNumitron(BlockEntityRBMKNumitron numitron) {
    ...
    protected void init() {
        ...
        for (int i = 0; i < DISPLAYS; i++) {
            BlockEntityRBMKNumitron.DisplayUnit unit = numitron.displays[i];
            label[i] =
                    field(
                            guiLeft + 27 + oX,
            ...
            shortenNumber[i] = unit.shortenNumber;
            leadingZeroes[i] = unit.leadingZeroes;
        }
    }
    ...
    public void extractRenderState(
        ...
        Component name = Component.translatable("container.rbmkNumitron");
        graphics.text(
                font,
                name,
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        for (int i = 0; i < DISPLAYS; i++) {
            ...
            if (guiLeft + SHORTEN_X <= x
                    && guiLeft + SHORTEN_X + TOGGLE_SIZE > x
                    && guiTop + i * ROW_PITCH + 53 < y
                    && guiTop + i * ROW_PITCH + 53 + TOGGLE_SIZE >= y) {
                shortenNumber[i] = !shortenNumber[i];
                click(0.5F + (shortenNumber[i] ? 0.25F : 0F));
                return true;
            }

            if (guiLeft + LEADING_X <= x
                    && guiLeft + LEADING_X + TOGGLE_SIZE > x
                    && guiTop + i * ROW_PITCH + 53 < y
                    && guiTop + i * ROW_PITCH + 53 + TOGGLE_SIZE >= y) {
                leadingZeroes[i] = !leadingZeroes[i];
                click(0.5F + (leadingZeroes[i] ? 0.25F : 0F));
                return true;
            }
        }
    ...
    private void sendSettings() {
        ...
        for (int i = 0; i < DISPLAYS; i++) {
            ...
            if (shortenNumber[i]) shortenMask |= (byte) (1 << i);
            if (leadingZeroes[i]) leadingMask |= (byte) (1 << i);
        }
        data.putByte("active", activeMask);
        data.putByte("polling", pollingMask);
        data.putByte("shorten_number", shortenMask);
        data.putByte("leading_zeroes", leadingMask);

        for (int i = 0; i < DISPLAYS; i++) {
            data.putString("label" + i, label[i].getValue());
            data.putString("rtty" + i, rtty[i].getValue());
        }
        Services.NETWORK.sendToServer(new NbtControlPayload(numitron.getBlockPos(), data));
    }
