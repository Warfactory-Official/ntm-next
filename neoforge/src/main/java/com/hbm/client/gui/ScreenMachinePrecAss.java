...
public class ScreenMachinePrecAss extends ScreenInfoContainer<MenuMachinePrecAss> {

    private static final Identifier TEXTURE = Library.id("textures/gui/processing/gui_precass.png");
    ...
    private static final int IN_X0 = 8, IN_Y0 = 27, IN_STRIDE = 18, IN_COLS = 3;
    private static final int TANK_X_IN = 8, TANK_X_OUT = 80, TANK_Y = 99, TANK_W = 52, TANK_H = 16;

    public ScreenMachinePrecAss(MenuMachinePrecAss menu, Inventory inventory, Component title) {
    ...
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (power > 0 && maxPower > 0) {
            int filled = (int) (power * POWER_H / maxPower);
            if (filled > 0) {
                graphics.blit(
                        RenderPipelines.GUI_TEXTURED,
                        TEXTURE,
        ...
        BlockEntityMachinePrecAss be = assembler();
        ...
        if (recipe != null && recipe.inputItem != null) {
            for (int i = 0; i < recipe.inputItem.length; i++) {
                if (be.getItem(BlockEntityMachinePrecAss.SLOT_INPUT_START + i).isEmpty()) {
                    graphics.item(
    ...
    protected void extractTooltip(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        GenericRecipe recipe = assembler().selectedRecipe();
        if (recipe != null) {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (event.button() == 0
            ...
            String pool =
                    ItemBlueprints.grabPool(be.getItem(BlockEntityMachinePrecAss.SLOT_BLUEPRINT));
            GUIScreenRecipeSelector.openSelector(
                    PrecAssRecipes.INSTANCE, be.getBlockPos(), current, 0, pool, this);
            playClick();
            return true;
        }
    ...
    private BlockEntityMachinePrecAss assembler() {
