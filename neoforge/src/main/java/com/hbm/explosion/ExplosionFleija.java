...
import com.hbm.blocks.generic.BlockStatueElbF;
    ...
    private void breakColumn(int x, int z) {
        ...
        if (dist > 0) {
            ...
            for (int y = (int) (dist / this.explosionCoefficient2);
                ...
                if (state.getBlock() instanceof BlockStatueElbF) continue;

                if (this.posY + y <= this.worldObj.getMinY()) continue;
                this.worldObj.setBlock(pos, Blocks.AIR.defaultBlockState(), 3);
            }
        }
    }

    public void saveToNbt(ValueOutput nbt, String name) {
        ...
        if (detonator != null) {
            nbt.store(name + "detonator", UUIDUtil.CODEC, detonator);
        }
    }

    public void readFromNbt(ValueInput nbt, String name) {
        ...
        detonator = nbt.read(name + "detonator", UUIDUtil.CODEC).orElse(null);
    }
}
