...
public class EntityCloudFleijaRainbow extends Entity {
    ...
    public EntityCloudFleijaRainbow(
            EntityType<? extends EntityCloudFleijaRainbow> type, Level level) {
        super(type, level);
        this.age = 0;
        this.scale = 0;
    }

    public static EntityCloudFleijaRainbow statFac(
            Level level, int maxAge, double x, double y, double z) {
        EntityCloudFleijaRainbow cloud =
                new EntityCloudFleijaRainbow(ModEntities.CLOUD_RAINBOW.get(), level);
        cloud.setPos(x, y, z);
