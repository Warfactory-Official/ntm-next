...
public class EntityCloudFX extends Entity {

    private int particleAge;
    private int maxAge;

    public EntityCloudFX(EntityType<? extends EntityCloudFX> type, Level level) {
        super(type, level);
        noPhysics = false;
    }

    public EntityCloudFX(
            Level level,
            double x,
            double y,
            double z,
            ...
            double motionY,
            double motionZ) {
        this(ModEntities.CLOUD_FX.get(), level);
        setPos(x, y, z);
    ...
    public void tick() {
        ...
        if (!level().isClientSide() && random.nextInt(50) == 0)
            ExplosionChaos.chlorine(level(), getX(), getY(), getZ(), 2D);
