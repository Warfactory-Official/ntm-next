...
public class EntityZirnoxDebris extends Entity {

    private static final EntityDataAccessor<Integer> TYPE =
            SynchedEntityData.defineId(EntityZirnoxDebris.class, EntityDataSerializers.INT);

    private static final int RAD_DURATION = 60 * SharedConstants.TICKS_PER_SECOND;
    private static final int RAD_LEVEL_ELEMENT = 7;
    private static final int RAD_LEVEL_GRAPHITE = 4;

    public float rot;
    public float lastRot;

    public EntityZirnoxDebris(EntityType<? extends EntityZirnoxDebris> type, Level level) {
        super(type, level);
        this.rot = this.lastRot = this.random.nextFloat() * 360F;
    }

    public EntityZirnoxDebris(Level level, double x, double y, double z, DebrisType type) {
        this(ModEntities.ZIRNOX_DEBRIS.get(), level);
        setPos(x, y, z);
        setDebrisType(type);
    }
    ...
    public void tick() {
        ...
        if (!level.isClientSide()) {
            ...
            if ((type == DebrisType.CONCRETE || type == DebrisType.EXCHANGER) && d.y > 0) {
                Vec3 from = position();
            ...
            if (type == DebrisType.ELEMENT || type == DebrisType.GRAPHITE) {
                int amplifier = type == DebrisType.ELEMENT ? RAD_LEVEL_ELEMENT : RAD_LEVEL_GRAPHITE;
                for (LivingEntity e :
        ...
        if (!level().isClientSide() && isAlive()) {
            ItemStack give =
                    switch (getDebrisType()) {
                        case BLANK -> new ItemStack(ModItems.DEBRIS_METAL);
                        case ELEMENT -> new ItemStack(ModItems.DEBRIS_ELEMENT);
                        case SHRAPNEL -> new ItemStack(ModItems.DEBRIS_SHRAPNEL);
                        case GRAPHITE -> new ItemStack(ModItems.DEBRIS_GRAPHITE);
                        case CONCRETE -> new ItemStack(ModItems.DEBRIS_CONCRETE);
                        case EXCHANGER -> new ItemStack(ModItems.DEBRIS_EXCHANGER);
                    };
    ...
    public int getLifetime() {
        return switch (getDebrisType()) {
            case BLANK -> 3 * SharedConstants.TICKS_PER_MINUTE;
            case ELEMENT -> 10 * SharedConstants.TICKS_PER_MINUTE;
            case SHRAPNEL, GRAPHITE -> 15 * SharedConstants.TICKS_PER_MINUTE;
            case CONCRETE, EXCHANGER -> 60 * SharedConstants.TICKS_PER_SECOND;
        };
    }
